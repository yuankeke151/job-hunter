...
场景B：双方均有消息
  ...
  否则 → call_ai(jd, need_self_promo, need_reply)：
    先发 self_promo（若 need_self_promo），延迟 2-3s，再发 reply（若 need_reply）
```

`_send_self_promo` 内部统一调用 `call_ai(need_reply=False, need_self_promo=True)` 并发送返回的 `self_promo`（原 `call_ai_self_promo` 已合并入 `call_ai`，按 `need_self_promo`/`need_reply` 标志位决定生成哪些字段）。
...
```
1. 轮询等待 .boss-popup__wrapper 出现（最多 5 秒，超时返回 False）
2. 找 span.resume-name 中含「袁柯」的项 → cdp_click 选中（找不到返回 False）
3. 等待 0.8s（选中状态渲染）
4. 找 .btn-confirm（text='发送'，找不到则在弹窗内按文字兜底查找）→ cdp_click 确认（找不到返回 False）
5. 等待 1.5-2.5s → 返回 True
```
