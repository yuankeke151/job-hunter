...
- **薪资字段**：BOSS直聘用 `kanzhun-mix` 自定义字体混淆薪资数字（私用区 Unicode 0xE031–0xE03A），读取到乱码，**暂不解析薪资**
...
│       └── cdp_utils.py         # CDP 底层工具函数 + read_messages
├── resume/
│   ├── 袁柯.pdf                 # 原始简历
...
## 运行参数（job_scanner.py 顶部常量）

| 常量 | 默认值 | 说明 |
|------|--------|------|
| `TARGET_CITY` | `"北京"` | 目标城市，非此城市只入库不解析 |
| `MAX_GREET` | `10` | 单次运行最多打招呼数量，达到后停止 |
| `SCORE_THRESHOLD` | `70` | AI 匹配分阈值（在 `config.py`），≥70 才推荐投递 |
| `SCROLL_DELTA` | `2000` | 每次滚动像素 |
| `SCROLL_WAIT` | `2.5` | 滚动后等待新卡片加载的秒数 |
| `STALE_LIMIT` | `2` | 连续无新卡片次数达到此值则判定末页停止 |
...
**停止条件**：打招呼总数达到 `MAX_GREET` 时，设置 `stop_scanning=True`，通过双层 `break` 退出，进入汇总。
...
调用 `analyzer.analyze_job(company, name, jd)`：
...
条件：`should_apply = True`。记录 `url_before`，CDP 点击 `.op-btn-chat`，等待 1–1.5 秒，检测结果：
...
| 当前会话信息 | `window.chat.communicating`（含 name/companyName/encryptJobId/jobName 等） |
...
| `src/shared/cdp_utils.py` | `evaluate` / `cdp_click` / `random_delay` / `read_messages`（移此以消除循环依赖） |
...
```javascript
// modules/cdp_utils.py _JS_READ_MESSAGES 中的分类逻辑
